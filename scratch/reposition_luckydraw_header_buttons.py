...
# Redesign Step 1 header to put all action buttons in a horizontal row below the subtitle
new_header_layout = """                        <!-- Second Row: Subtitle -->
...
# Replace the old Row 2 container
import re
pattern = re.compile(r'<!-- Second Row: Subtitle \+ Back -->.*?</div>\s+</div>\s+</div>', re.DOTALL)
match = pattern.search(content)
if match:
    content = content[:match.start()] + new_header_layout + content[match.end():]

with open(path, 'w', encoding='utf-8', newline='\r\n') as f:
    f.write(content)

print("LuckyDraw: Header layout updated to horizontal row below subtitle.")
