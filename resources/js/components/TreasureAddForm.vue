        ...
        <div class="relative w-full max-w-2xl bg-white rounded-t-[32px] shadow-[0_-10px_40px_rgba(0,0,0,0.1)] overflow-hidden animate-slide-up flex flex-col max-h-[90vh]">
            ...
            <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-white sticky top-0 z-10">
                <h3 class="text-lg font-bold text-slate-800">
                    {{ mode === 'single' ? (initialData.id ? '修改法寶' : '逐筆新增法寶') : '多筆一次新增' }}
                </h3>
            ...
            <div class="flex-1 overflow-y-auto p-6 space-y-4 custom-scrollbar">
                ...
                <div v-if="mode === 'single'" class="space-y-4">
                    <div class="grid grid-cols-2 gap-4">
                        <div class="space-y-1.5">
                            <label class="text-xs font-bold text-slate-500 uppercase ml-1">登記日期</label>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input v-model="form.name" type="text" placeholder="例如：降魔杵" class="w-full rounded-2xl border-slate-100 bg-slate-50 p-3 text-sm focus:ring-2 focus:ring-indigo-500/20">
                    </div>

                    <div class="space-y-1.5">
                        <label class="text-xs font-bold text-slate-500 uppercase ml-1">法寶用意</label>
                        <input v-model="form.purpose" type="text" placeholder="請輸入法寶用意" class="w-full rounded-2xl border-slate-100 bg-slate-50 p-3 text-sm focus:ring-2 focus:ring-indigo-500/20">
                    </div>

                    <div class="space-y-1.5">
                        <label class="text-xs font-bold text-slate-500 uppercase ml-1">求寶方式</label>
                        <input v-model="form.acquisition_method" type="text" placeholder="請輸入求寶方式" class="w-full rounded-2xl border-slate-100 bg-slate-50 p-3 text-sm focus:ring-2 focus:ring-indigo-500/20">
                    </div>
                ...
                <div v-if="mode === 'batch'" class="space-y-4">
                    <div class="grid grid-cols-2 gap-4">
                        <div class="space-y-1.5">
                            <label class="text-xs font-bold text-slate-500 uppercase ml-1">預設日期</label>
                            <input v-model="batchDate" type="date" class="w-full rounded-2xl border-slate-100 bg-slate-50 p-3 text-sm">
                        </div>
                        <div class="space-y-1.5">
                    ...
                    <div class="p-4 bg-indigo-50 rounded-2xl space-y-3">
                        ...
                        <textarea v-model="batchInput" rows="10" 
                            ...
                            placeholder="支援複製 LINE 內容、Excel 清單直接貼入..."></textarea>
                        <input type="file" ref="fileInput" class="hidden" @change="handleFileUpload" accept=".txt,.csv,.xls,.xlsx,.doc,.docx">
                    </div>
                </div>
            </div>
...
watch(() => props.initialData, (newVal) => {
    form.value = { ...newVal };
    if (props.mode === 'batch') {
        batchMasterId.value = newVal.master_id;
    }
}, { deep: true });
...
// Sync batchInput back if needed (though batch parsing usually happens on save)
defineExpose({
    setBatchInput: (val) => batchInput.value = val
});
</script>
