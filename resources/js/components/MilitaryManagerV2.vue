    ...
    <div class="bg-slate-100 md:bg-white h-full flex flex-col relative overflow-clip">
        ...
        <div v-if="showSearch" class="px-2 py-2 animate-fade-in">
            <div class="relative group">
                ...
                <input v-model.lazy="searchQuery" @keyup.enter="$event.target.blur()" type="text" placeholder="搜尋項目..." 
            ...
            <div v-if="!currentFolder && !addMode && !showSearch && !showFullTotal" class="flex-1 overflow-y-auto custom-scrollbar bg-slate-50/30">
                <div class="px-6 pb-24 flex flex-col items-center justify-center gap-y-12 mt-[25px] max-w-4xl mx-auto">
                    <button v-for="folder in filteredFolders" :key="folder.id" 
                        ...
                        class="flex flex-col items-center justify-center p-2 active:scale-95 transition-all group relative w-full max-w-[480px]">
                    ...
                    <div v-else class="flex flex-col flex-1 px-1">
                            ...
                            <div v-for="group in dateGroupsData" :key="group.know_date || 'historical'"
                                ...
                                <div class="flex items-center z-10">
                                    <div class="flex flex-col">
                                        <span class="font-outfit tracking-wider text-slate-800 military-label" style="color: #1e293b !important;">{{ group.know_date ? formatDate(group.know_date) : '原始數量' }}</span>
                                    </div>
                                </div>
                                <div class="flex items-center space-x-2 z-10">
                                    <div v-if="group.total_qty" class="flex items-center">
                                        <span class="text-black drop-shadow-sm military-value">{{ formatArmyTotal(group.total_qty) }}</span>
                                    </div>
                        ...
                        <template v-else>
                            ...
                            <div v-if="expandedDate && focusedId === null && !searchQuery"
                                ...
                                <span class="font-outfit tracking-wider text-slate-800 military-label" style="color: #1e293b !important;">{{ expandedDate }}</span>
                            </div>
