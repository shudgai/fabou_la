<template>
    <div v-if="show" class="fixed inset-0 z-[70] flex items-end md:items-center justify-center px-0">
        ...
        <div class="relative w-full h-full md:h-auto md:max-h-[90vh] md:max-w-2xl bg-white md:rounded-[32px] shadow-[0_-10px_40px_rgba(0,0,0,0.1)] overflow-hidden animate-slide-up flex flex-col">
            ...
            <div class="flex-1 overflow-y-auto px-[10px] pt-[5px] pb-4 space-y-[15px] custom-scrollbar bg-white">
                ...
                <div class="space-y-[15px]">
                    ...
                    <div class="space-y-0.5 mt-[-5px]">
                        <label class="text-[14px] font-black text-slate-400 uppercase ml-1">得知日期</label>
                        <div @click="activeDate = 'know_date'" 
                            ...
                            <span :class="form.know_date ? 'text-slate-900' : 'text-slate-400'" class="text-[18px] leading-tight font-black">
                    ...
                    <div class="grid grid-cols-2 gap-[15px]">
                        <div class="space-y-0.5">
                            <label class="text-[14px] font-black text-slate-400 uppercase ml-1">法號</label>
                            <input v-model="form.user_name" type="text" list="user-list" placeholder="輸入法號" class="w-full h-[36px] rounded-lg border border-slate-200 bg-white px-2 focus:ring-0 outline-none shadow-sm text-[18px] leading-tight text-slate-900 font-black">
                            <datalist id="user-list">
                                <option v-for="u in users" :key="u.id" :value="u.name"></option>
                            </datalist>
                        </div>
                        <div class="space-y-0.5">
                            <label class="text-[14px] font-black text-slate-400 uppercase ml-1">備註</label>
                            <input v-model="form.user_remarks" type="text" placeholder="親友/信眾" class="w-full h-[36px] rounded-lg border border-slate-200 bg-white px-2 focus:ring-0 outline-none shadow-sm text-[18px] leading-tight text-slate-900 font-black">
                        </div>
                    </div>
                    ...
                    <div :class="form.destination === '未處理' ? 'space-y-0.5' : 'grid grid-cols-2 gap-[15px]'">
                        <div v-if="form.destination !== '未處理'" class="space-y-0.5">
                            <label class="text-[14px] font-black text-slate-400 uppercase ml-1">處理日期</label>
                            <div @click="activeDate = 'process_date'" 
                                class="w-full h-[36px] rounded-lg bg-white border border-slate-200 px-2 flex items-center justify-start space-x-2 cursor-pointer shadow-sm overflow-hidden">
                                <span :class="form.process_date ? 'text-slate-900' : 'text-slate-400'" class="text-[18px] leading-tight font-black">
                        ...
                        <div class="space-y-0.5 relative">
                            <label class="text-[14px] font-black text-slate-400 uppercase ml-1">處理結果</label>
                            ...
                            <div @click="showResultPicker = !showResultPicker" 
                                ...
                                <span class="font-black text-[18px] leading-tight" :class="[
                    ...
                    <div v-if="form.destination === '黑曜軍'" class="grid grid-cols-2 gap-[5px] animate-fade-in">
                        <div class="space-y-0.5">
                            <label class="text-[14px] font-black text-slate-400 uppercase ml-1">閻尊</label>
                            <input v-model.number="form.remarks.yan_zun" type="number" placeholder="0" class="w-full h-[36px] rounded-lg border border-slate-200 bg-white px-2 focus:ring-0 outline-none shadow-sm text-[18px] leading-tight text-slate-900 font-black">
                        </div>
                        <div class="space-y-0.5">
                            <label class="text-[14px] font-black text-slate-400 uppercase ml-1">閻闇</label>
                            <input v-model.number="form.remarks.yan_an" type="number" placeholder="0" class="w-full h-[36px] rounded-lg border border-slate-200 bg-white px-2 focus:ring-0 outline-none shadow-sm text-[18px] leading-tight text-slate-900 font-black">
                        </div>
                    </div>
                    ...
                    <div v-if="form.destination === '耀紫軍'" class="grid grid-cols-2 gap-[5px] animate-fade-in">
                        <div class="space-y-0.5">
                            <label class="text-[14px] font-black text-slate-400 uppercase ml-1">龍勝</label>
                            <input v-model.number="form.remarks.long_sheng" type="number" placeholder="0" class="w-full h-[36px] rounded-lg border border-slate-200 bg-white px-2 focus:ring-0 outline-none shadow-sm text-[18px] leading-tight text-slate-900 font-black">
                        </div>
                        <div class="space-y-0.5">
                            <label class="text-[14px] font-black text-slate-400 uppercase ml-1">龍戰</label>
                            <input v-model.number="form.remarks.long_zhan" type="number" placeholder="0" class="w-full h-[36px] rounded-lg border border-slate-200 bg-white px-2 focus:ring-0 outline-none shadow-sm text-[18px] leading-tight text-slate-900 font-black">
                        </div>
                    </div>

                    <!-- Row 5: 備註文字 -->
                    <div class="space-y-1">
                        <label class="text-[14px] font-black text-slate-400 uppercase ml-1">備註文字</label>
                        <input v-model="form.remarks_text" type="text" placeholder="輸入相關備註..." class="w-full h-[36px] rounded-lg border border-slate-100 bg-slate-50/50 px-3 focus:ring-0 outline-none shadow-sm text-[18px] text-slate-900 font-black">
                    </div>
            ...
            <!-- Footer Action -->
            <div class="p-3 pb-10 border-t border-slate-100 bg-white">
                <button 
                    @click="handleSave" 
                    class="w-full bg-indigo-600 text-white font-black p-[8px] text-[18px] rounded-2xl shadow-lg shadow-indigo-200 hover:bg-indigo-700 active:scale-[0.98] transition-all"
                >
                    {{ editingId ? '確認修改' : '確認載錄' }}
                </button>
            </div>
            ...
            <div v-if="showResultPicker" class="absolute inset-0 bg-white z-[100] p-8 flex flex-col animate-fade-in md:rounded-[32px]">
                <div class="flex items-center justify-between mb-4">
                    <span class="text-[15px] font-black text-slate-400 uppercase tracking-widest">請選取處理結果</span>
                    <button @click="showResultPicker = false" class="text-slate-400 p-2"><svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M6 18L18 6M6 6l12 12" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"/></svg></button>
                </div>
                <div class="grid grid-cols-2 gap-x-4 flex-1">
                    <!-- Left Column -->
                    <div class="space-y-4">
                        <div v-for="opt in ['未處理', '暫驅離', '殲滅']" :key="opt"
                            @click="form.destination = opt; onDestinationChange(); showResultPicker = false"
                            class="cursor-pointer font-black text-[24px] leading-tight transition-all active:scale-95 py-1.5"
                            :class="[
