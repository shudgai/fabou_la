            ...
            <div class="flex-1 overflow-y-auto px-[10px] pt-[5px] pb-4 space-y-2 custom-scrollbar bg-white">
                ...
                <div class="space-y-2">
                    ...
                    <div class="space-y-0.5 mt-[-5px]">
                        <label class="text-[14px] font-bold text-black uppercase ml-1">得知日期</label>
                    ...
                    <div class="grid grid-cols-2 gap-2">
                        <div class="space-y-0.5">
                            ...
                            <input v-model="form.user_name" type="text" list="user-list" placeholder="輸入法號" class="w-full h-[26px] rounded-lg border border-slate-200 bg-white px-2 focus:ring-0 outline-none shadow-sm text-[16px] leading-tight">
                            <datalist id="user-list">
                                <option v-for="u in users" :key="u.id" :value="u.name"></option>
                            </datalist>
                        </div>
                        <div class="space-y-0.5">
                            <label class="text-[14px] font-bold text-black uppercase ml-1">備註</label>
                    ...
                    <div :class="form.destination === '未處理' ? 'space-y-0.5' : 'grid grid-cols-2 gap-2'">
                        <div v-if="form.destination !== '未處理'" class="space-y-0.5">
                            <label class="text-[14px] font-bold text-black uppercase ml-1">處理日期</label>
                        ...
                        <div class="space-y-0.5 relative">
                            <label class="text-[14px] font-bold text-black uppercase ml-1">處理結果</label>
                            
                            <!-- Trigger Button -->
                            <div @click="showResultPicker = !showResultPicker" 
                                class="w-full h-[26px] rounded-lg border border-slate-200 bg-white px-2 flex items-center justify-between cursor-pointer shadow-sm active:bg-slate-50 transition-all overflow-hidden">
                                <span class="font-bold text-[16px] leading-tight" :class="[
                    ...
                    <div v-if="form.destination === '黑曜軍'" class="grid grid-cols-2 gap-2 animate-fade-in">
                        <div class="space-y-0.5">
                            <label class="text-[14px] font-bold text-slate-400 uppercase ml-1">閻尊</label>
                            <input v-model="form.remarks.yan_zun" type="number" placeholder="0" class="w-full h-[26px] rounded-lg border border-slate-200 bg-white px-2 focus:ring-0 outline-none shadow-sm text-[16px] leading-tight">
                        </div>
                        <div class="space-y-0.5">
                            <label class="text-[14px] font-bold text-slate-400 uppercase ml-1">閻闇</label>
                            <input v-model="form.remarks.yan_an" type="number" placeholder="0" class="w-full h-[26px] rounded-lg border border-slate-200 bg-white px-2 focus:ring-0 outline-none shadow-sm text-[16px] leading-tight">
                        </div>
                    </div>

                    <!-- Row 4.2: Sub-options for 耀紫軍 -->
                    <div v-if="form.destination === '耀紫軍'" class="grid grid-cols-2 gap-2 animate-fade-in">
                        <div class="space-y-0.5">
                            <label class="text-[14px] font-bold text-slate-400 uppercase ml-1">龍勝</label>
                            <input v-model="form.remarks.long_sheng" type="number" placeholder="0" class="w-full h-[26px] rounded-lg border border-slate-200 bg-white px-2 focus:ring-0 outline-none shadow-sm text-[16px] leading-tight">
                        </div>
                        <div class="space-y-0.5">
                            <label class="text-[14px] font-bold text-slate-400 uppercase ml-1">龍戰</label>
                            <input v-model="form.remarks.long_zhan" type="number" placeholder="0" class="w-full h-[26px] rounded-lg border border-slate-200 bg-white px-2 focus:ring-0 outline-none shadow-sm text-[16px] leading-tight">
                        </div>
                    </div>
                ...
                <div class="grid grid-cols-2 gap-x-12 flex-1">
                    ...
                    <div class="space-y-8">
                        <div v-for="opt in ['未處理', '暫驅離', '殲滅']" :key="opt"
                            ...
                            class="cursor-pointer font-bold text-[13px] transition-all active:scale-95 py-1"
                            :class="[
                    ...
                    <div class="space-y-7">
                        <div v-for="opt in ['九天', '黑曜軍', '虎賁軍', '虎甲軍', '耀紫軍']" :key="opt"
                            ...
                            :class="[
                                form.destination === opt ? 'text-indigo-600 underline underline-offset-4' : 'text-slate-900 opacity-60',
                                opt === '九天' && form.destination !== opt ? 'text-red-500 font-bold' : 
                                opt === '耀紫軍' && form.destination !== opt ? 'text-purple-500 font-bold' : ''
                            ]"
                        >
                            {{ opt }}
                        </div>
