    ...
    <div v-if="mode" class="fixed inset-0 z-[3500] flex items-end md:items-center justify-center px-0">
        ...
        <div class="relative w-full h-full md:h-auto md:max-h-[95vh] md:max-w-xl bg-white md:rounded-[32px] shadow-[0_-10px_40px_rgba(0,0,0,0.1)] overflow-hidden animate-slide-up flex flex-col pb-[7vh]">
            ...
            <div class="px-6 pt-4 flex space-x-1">
                <button @click="localMode = 'single'" :class="[localMode === 'single' ? 'bg-blue-600 !text-white shadow-md' : 'bg-slate-100 text-slate-400 hover:bg-slate-200']" class="flex-1 py-[12px] rounded-xl text-[16px] font-black transition-all whitespace-nowrap" :style="localMode === 'single' ? 'color: white !important;' : ''">逐筆登錄</button>
                <button @click="localMode = 'batch'" :class="[localMode === 'batch' ? 'bg-blue-600 !text-white shadow-md' : 'bg-slate-100 text-slate-400 hover:bg-slate-200']" class="flex-1 py-[12px] rounded-xl text-[16px] font-black transition-all whitespace-nowrap" :style="localMode === 'batch' ? 'color: white !important;' : ''">多筆載錄</button>
            </div>
            ...
            <div class="absolute bottom-[7vh] left-0 right-0 md:relative md:bottom-0 p-4 pb-6 bg-white backdrop-blur-md border-t border-slate-50 z-[10] shadow-[0_-10px_30px_rgba(0,0,0,0.02)]">
                ...
                <button v-else @click="handleSubmit" :disabled="isSaving || parsedItemsCount === 0" 
                    class="w-full bg-blue-600 !text-white h-[55px] rounded-2xl font-black text-[20px] shadow-lg shadow-blue-100 active:scale-95 transition-all flex items-center justify-center tracking-widest disabled:opacity-50" style="color: white !important;">
