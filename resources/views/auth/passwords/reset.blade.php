...
<div class="min-h-[calc(100vh-64px)] bg-slate-50 flex items-center justify-center p-6 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] bg-fixed">
    <div class="max-w-md w-full animate-fade-in-up">
        ...
        <div class="text-center mb-8">
            ...
            <h2 class="text-3xl font-black text-slate-900 tracking-tight font-outfit">設置新密碼</h2>
            <p class="text-slate-500 text-sm mt-2">請為您的帳號設定一個安全的新密碼</p>
        </div>
        ...
        <div class="bg-white rounded-[40px] shadow-[0_20px_70px_rgba(0,0,0,0.05)] border border-slate-100 p-8 md:p-10 relative overflow-hidden group">
            ...
            <form method="POST" action="{{ route('password.update') }}" class="space-y-6">
                ...
                <!-- Email field (Hidden or readonly as it's usually pre-filled) -->
                <div class="space-y-2">
                    ...
                    <div class="relative">
                        ...
                        <input id="email" type="email" name="email" value="{{ $email ?? old('email') }}" required autocomplete="email" readonly
                            class="w-full pl-12 pr-4 py-4 bg-slate-50 border-none rounded-2xl text-base focus:ring-2 focus:ring-indigo-500/20 transition-all text-slate-500 cursor-not-allowed">
                    </div>
                    @error('email')
                ...
                <div class="space-y-2">
                    ...
                    <div class="relative">
                        ...
                        <input id="password" type="password" name="password" required autocomplete="new-password" autofocus
                            class="w-full pl-12 pr-4 py-4 bg-slate-50 border-none rounded-2xl text-base focus:ring-2 focus:ring-indigo-500/20 transition-all placeholder-slate-300 @error('password') ring-2 ring-red-500/20 @enderror"
                            placeholder="輸入新密碼">
                    </div>
                    @error('password')
                        <p class="text-red-500 text-sm font-bold mt-1 ml-1">{{ $message }}</p>
                ...
                <div class="space-y-2">
                    ...
                    <div class="relative">
                        ...
                        <input id="password-confirm" type="password" name="password_confirmation" required autocomplete="new-password"
                            class="w-full pl-12 pr-4 py-4 bg-slate-50 border-none rounded-2xl text-base focus:ring-2 focus:ring-indigo-500/20 transition-all placeholder-slate-300"
                            placeholder="再次輸入新密碼">
                    </div>
                </div>
